...
export default function ExitIntentPopup() {
  ...
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      if (e.clientY <= 5 && !localStorage.getItem("exitShown")) {
        setShow(true);
  ...
  return (
    <AnimatePresence>
      {show && (
        ...
        >
          <motion.div
            className="bg-card rounded-3xl p-8 md:p-10 max-w-md w-full shadow-2xl relative border border-border"
          ...
          >
            ...
            <div className="text-center">
              ...
              <Button
                className="accent-gradient border-0 text-accent-foreground w-full h-12 rounded-xl font-semibold shadow-md"
                asChild
                onClick={() => trackCTA("exit_intent_click")}
              >
